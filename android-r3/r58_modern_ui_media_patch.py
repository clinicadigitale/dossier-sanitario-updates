...
# Make the redesign immediately visible on launch while leaving navigation and clinical engines intact.
replacements = [
...
# The R58 home hero is deliberately unmistakable so a real-device test can confirm the new UI at a glance.
if 'private void addR58ModernHero()' not in main:
    anchor = '    private void addReleaseNotice() {'
    require(main, anchor, 'release notice insertion anchor')
...
# Put the redesigned hero before the legacy release note and dashboard cards.
render_anchor = '    private void renderPanoramica() {\n        addReleaseNotice();'
require(main, render_anchor, 'Panoramica anchor')
main = main.replace(render_anchor, '    private void renderPanoramica() {\n        addR58ModernHero();\n        addReleaseNotice();', 1)

# Modernise section dialog title and top quick actions without changing their destinations.
...
# Media screen: keep the R56 functionality, but bring it into the same visible design language.
...
# Align inherited version assertions only.
...
print('R58 visible modern UI + imaging surface patch applied')
